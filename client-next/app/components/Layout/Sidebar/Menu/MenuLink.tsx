  ...
  return (
    <li>
      <Link href={item.link}>
        <a 
          className={cn(
            styles.menu_link, {[styles.active]: asPath === item.link}
          )}
        > 
          <div 
            className={cn( 
              styles.content, {[styles.image]: !!item.image}
            )}
          >
            {item.icon && <item.icon/> }
            {item.image && <Image src={item.image} alt={item.title} width={40} height={40}/> }
          </div>
